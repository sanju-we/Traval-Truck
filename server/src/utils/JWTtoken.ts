...
export class JWT implements IJWT {
  ...
  async generateToken(payload: { id:string; role: string }): Promise<{
    ...
    try {
      logger.info(`payload from JWT->generateToken : ${payload}`)
  ...
  async generateResetToken(user: IUser): Promise<{ resetToken: string; resetLink: string }> {
    try {
  ...
  async verifyRefreshToken(refreshToken: string):Promise<{ id: string; role: string; }> {
      ...
      const decoded: { id: string; role: string; } = JSON.parse(JSON.stringify(jwt.verify(refreshToken, this.JWT_SECRET)));
