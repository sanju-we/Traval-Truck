...
export class RestaurantProfileService implements IRestaurantProfileService {
  constructor(
    @inject('IRestaurantAuthRepository') private readonly _restaurantAuthRepo: IRestaurantAuthRepository
  ) { }
  ...
  async updateDocuments(id: string, files: { [fieldname: string]: Express.Multer.File[]; }): Promise<vendorRequestDTO> {
    let update;
