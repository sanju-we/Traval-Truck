...
let AdminVendorRepository = class AdminVendorRepository {
    ...
    async findAllUsers(page, limit, status, role, search) {
        ...
        const vendorDTO = [
            ...agencyData.map(vendor_response_dto_1.toVendorRequestDTO),
            ...hotelData.map(vendor_response_dto_1.toVendorRequestDTO),
            ...restaurantData.map(vendor_response_dto_1.toVendorRequestDTO),
        ];
        const allUserDTO = [...userData.map(user_profile_1.toUserProfileDTO)];
