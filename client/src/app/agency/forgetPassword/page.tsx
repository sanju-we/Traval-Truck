...
export default function ForgotPasswordPage() {
  ...
  const handleForgotPassword = async () => {
    ...
    setIsLoading(true);
    try {
      const data = await AGENCY_API_METHODS.forgotPassword(email) as ApiResponse;

      if (data && data.success) {
        toast.success(`success`);
        router.push('/agency/forgetPassword/sent');
      } else {
    ...
    <div className="flex flex-col items-center justify-center bg-white">
      ...
      <main className="min-h-screen flex flex-col items-center w-full max-w-md mt-10 px-6">
        ...
        <p className="mt-6 text-sm text-gray-500">
          ...
          <a href="/agency/login" className="text-purple-600 hover:underline">
            Log in
          </a>
        </p>
      </main>
